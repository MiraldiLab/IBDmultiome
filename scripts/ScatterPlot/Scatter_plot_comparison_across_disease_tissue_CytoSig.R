...
dir_out<-file.path(results_dir,'Downsampled_fgsea_RNAEnrich_co_viz/CytoSig/ScatterPlot_comparison')
file_gsea_res_CD_R<-file.path(results_dir,'Downsample_statWald/CytoSig/CD_R/CytoSig_I_vs_NI_-log10_Padj_all_cytokine_df_frac_L_R_by_pathway.txt')
file_gsea_res_CD_TI<-file.path(results_dir,'Downsample_statWald/CytoSig/CD_TI/CytoSig_I_vs_NI_-log10_Padj_all_cytokine_df_frac_L_R_by_pathway.txt')
file_gsea_res_UC_R<-file.path(results_dir,'Downsample_statWald/CytoSig/UC_R/CytoSig_I_vs_NI_-log10_Padj_all_cytokine_df_frac_L_R_by_pathway.txt')
file_gsea_res_CD_R_RNAEnrich<-file.path(results_dir,'Downsample_RNAEnrich/CytoSig/CD_R/CytoSig_I_vs_NI_-log10_Padj_all_cytokine_df_frac_L_R_by_pathway.txt')
file_gsea_res_CD_TI_RNAEnrich<-file.path(results_dir,'Downsample_RNAEnrich/CytoSig/CD_TI/CytoSig_I_vs_NI_-log10_Padj_all_cytokine_df_frac_L_R_by_pathway.txt')
file_gsea_res_UC_R_RNAEnrich<-file.path(results_dir,'Downsample_RNAEnrich/CytoSig/UC_R/CytoSig_I_vs_NI_-log10_Padj_all_cytokine_df_frac_L_R_by_pathway.txt')
#load results 
gsea_res_CD_R<-read.table(file_gsea_res_CD_R,sep='\t',header=T,check.names=F)
gsea_res_CD_TI<-read.table(file_gsea_res_CD_TI,sep='\t',header=T,check.names=F)
gsea_res_UC_R<-read.table(file_gsea_res_UC_R,sep='\t',header=T,check.names=F)
gsea_res_CD_R_RNAEnrich<-read.table(file_gsea_res_CD_R_RNAEnrich,sep='\t',header=T,check.names=F)
gsea_res_CD_TI_RNAEnrich<-read.table(file_gsea_res_CD_TI_RNAEnrich,sep='\t',header=T,check.names=F)
gsea_res_UC_R_RNAEnrich<-read.table(file_gsea_res_UC_R_RNAEnrich,sep='\t',header=T,check.names=F)
#modify the fgsea results based on RNAEnrich CD R
gsea_res_CD_R$ContextPathway<-paste(gsea_res_CD_R$Context,gsea_res_CD_R$Pathway,sep='_')
gsea_res_CD_R_RNAEnrich$ContextPathway<-paste(gsea_res_CD_R_RNAEnrich$Context,gsea_res_CD_R_RNAEnrich$Pathway,sep='_')
sig_up_context_pathway_fgsea_CD_R<-gsea_res_CD_R[gsea_res_CD_R$`-log10Padj`>1,'ContextPathway']
sig_dn_context_pathway_fgsea_CD_R<-gsea_res_CD_R[gsea_res_CD_R$`-log10Padj`<(-1),'ContextPathway']
sig_up_context_pathway_RNAEnrich_CD_R<-gsea_res_CD_R_RNAEnrich[gsea_res_CD_R_RNAEnrich$`-log10Padj`>1,'ContextPathway']
sig_dn_context_pathway_RNAEnrich_CD_R<-gsea_res_CD_R_RNAEnrich[gsea_res_CD_R_RNAEnrich$`-log10Padj`<(-1),'ContextPathway']
...
gsea_res_CD_R[gsea_res_CD_R$ContextPathway%in%sig_disagree_CD_R,'-log10Padj']<-0
#modify the fgsea results based on RNAEnrich CD TI
gsea_res_CD_TI$ContextPathway<-paste(gsea_res_CD_TI$Context,gsea_res_CD_TI$Pathway,sep='_')
gsea_res_CD_TI_RNAEnrich$ContextPathway<-paste(gsea_res_CD_TI_RNAEnrich$Context,gsea_res_CD_TI_RNAEnrich$Pathway,sep='_')
sig_up_context_pathway_fgsea_CD_TI<-gsea_res_CD_TI[gsea_res_CD_TI$`-log10Padj`>1,'ContextPathway']
sig_dn_context_pathway_fgsea_CD_TI<-gsea_res_CD_TI[gsea_res_CD_TI$`-log10Padj`<(-1),'ContextPathway']
sig_up_context_pathway_RNAEnrich_CD_TI<-gsea_res_CD_TI_RNAEnrich[gsea_res_CD_TI_RNAEnrich$`-log10Padj`>1,'ContextPathway']
sig_dn_context_pathway_RNAEnrich_CD_TI<-gsea_res_CD_TI_RNAEnrich[gsea_res_CD_TI_RNAEnrich$`-log10Padj`<(-1),'ContextPathway']
...
gsea_res_CD_TI[gsea_res_CD_TI$ContextPathway%in%sig_disagree_CD_TI,'-log10Padj']<-0
#modify the fgsea results based on RNAEnrich CD R
gsea_res_UC_R$ContextPathway<-paste(gsea_res_UC_R$Context,gsea_res_UC_R$Pathway,sep='_')
gsea_res_UC_R_RNAEnrich$ContextPathway<-paste(gsea_res_UC_R_RNAEnrich$Context,gsea_res_UC_R_RNAEnrich$Pathway,sep='_')
sig_up_context_pathway_fgsea_UC_R<-gsea_res_UC_R[gsea_res_UC_R$`-log10Padj`>1,'ContextPathway']
sig_dn_context_pathway_fgsea_UC_R<-gsea_res_UC_R[gsea_res_UC_R$`-log10Padj`<(-1),'ContextPathway']
sig_up_context_pathway_RNAEnrich_UC_R<-gsea_res_UC_R_RNAEnrich[gsea_res_UC_R_RNAEnrich$`-log10Padj`>1,'ContextPathway']
sig_dn_context_pathway_RNAEnrich_UC_R<-gsea_res_UC_R_RNAEnrich[gsea_res_UC_R_RNAEnrich$`-log10Padj`<(-1),'ContextPathway']
...
gsea_res_UC_R[gsea_res_UC_R$ContextPathway%in%sig_disagree_UC_R,'-log10Padj']<-0
...
for (ix in 1:nrow(df_contrast)){
  ...
  file_out<-file.path(dir_out,paste0(curr_ident1,'_CD_R_vs_',curr_ident2,
                                     '_CD_TI_CytoSig_cytokine_response_enrichment_scatter_plot.pdf'))
...
ggplot(df_viz, aes(x = `-log10Padj(CD_R)`, y = `-log10Padj(UC_R)`)) +
  ...
  theme(plot.title = element_text(size = 14,hjust=0.5),
        ...
        axis.text = element_text(color = "black"),
        legend.position = "none")
file_out<-file.path(dir_out,paste0('Colonocyte_CD_R_vs_UC_R_CytoSig_cytokine_response_enrichment_scatter_plot.pdf'))
ggsave(file_out,height=5.5,width=5.8)
...
ggplot(df_viz, aes(x = `-log10Padj(CD_R)`, y = `-log10Padj(UC_R)`)) +
  ...
  theme(plot.title = element_text(size = 14,hjust=0.5),
        ...
        axis.text = element_text(color = "black"),
        legend.position = "none")
file_out<-file.path(dir_out,paste0('Macrophage_CD_R_vs_UC_R_CytoSig_cytokine_response_enrichment_scatter_plot.pdf'))
ggsave(file_out,height=5.5,width=5.8)
