# Downloaded by jwii 2025-06
# Supplemental code from original RNA-Enrich paper
# Lee et al. 2016 Bioinformatics (PMID: 26607492)
...
if (database == "custom") {
  if (!is.null(conceptList) & !is.null(nullsetList)) {
    ...
    xx <- xx[!is.na(xx)]
    nullset <- unique(as.character(unlist(xx)))
  } else {
  ...
  ## XX ADDED
  #cat("length(nullset) =", length(nullset), "\n")
  #cat("length(de_df$geneid) =", length(de_df$geneid), "\n")
  #missing_genes <- nullset[!nullset %in% de_df$geneid]
  #cat("length(missing_genes) =", length(missing_genes), "\n")

	#make empty d dataframe to initiate gpw creation
  missing_genes <- nullset[!nullset %in% de_df$geneid]
  d_allgenes <- data.frame(
      geneid = missing_genes,
      p.value = rep(NA_real_, length(missing_genes)),
      readcount = rep(0, length(missing_genes)),
      stringsAsFactors = FALSE
  )
  if (!is.null(direction)) {
      d_allgenes$direction <- rep(NA_real_, nrow(d_allgenes))
  }
  ...
  #cat("gpw rows:", nrow(gpw), "\n")
  #cat("NA x:", sum(is.na(gpw$weight * gpw$log10_p.value)), "\n")
  #cat("Inf x:", sum(is.infinite(gpw$weight * gpw$log10_p.value)), "\n")
  #cat("unique x:", length(unique(gpw$weight * gpw$log10_p.value)), "\n")
  summary(gpw$weight * gpw$log10_p.value)
  gpw$x_test <- gpw$weight * gpw$log10_p.value
  gpw <- gpw[is.finite(gpw$x_test), ]
	...
	for (i in 1:length(xx)) {
		...
		testm <- cbind(y = as.numeric(b_genes), x = gpw$x_test)
