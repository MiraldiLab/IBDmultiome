...
# input date object
file_gsea<-file.path(results_dir,paste('Downsample/1Factor_CellType_Downsample_statWald/CD_TI/CytoSig/gsea_IBD_multiome_CytoSig_I_vs_NI_Disease_Tissue_All.rds'))
file_sig_sets<-file.path(results_dir,paste('Downsample/1Factor_CellType_Downsample_statWald/CD_TI/CytoSig/sig_setsIBD_multiome_CytoSig_All.txt'))
...
# output directories
dir_out <- file.path(project_dir, "results","GSEA_RNA_Status", "Downsample_statWald", "CytoSig", "CD_TI")
...
colnames(df)<-c('Pathway','-log10Padj','NES','Context')
...
df_pathway<-df %>%
  group_by(Pathway, Context,`-log10Padj`, NES, Status) %>%
