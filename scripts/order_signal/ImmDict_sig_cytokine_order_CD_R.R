...
file_df_gsea_ImmDict<-file.path(results_dir,paste('Downsample_statWald/PROGENy_CD64_ImmDict/CD_R/ImmDict_I_vs_NI_-log10_Padj_all_cytokine_df_frac_L_R_by_pathway.txt'))
file_df_ImmDict_RNAEnrich<-file.path(results_dir,paste('Downsample_RNAEnrich/PROGENy_CD64_ImmDict/CD_R/ImmDict_I_vs_NI_-log10_Padj_all_cytokine_df_frac_L_R_by_pathway.txt'))
...
df<-read.table(file_df_gsea_ImmDict,sep='\t',header=T,check.names=F)
...
for (ix in sig_sets_ImmDict){
  ...
  curr_df_max_padj_cytokine_family<-data.frame(Cytokine=ix,Pathway=ix,
...
order_cytokine_family<-readLines(file.path(dir_out,paste0('sig_sets_ImmDict_cytokine_family_order.txt')))
...
file_out<-file.path(dir_out,paste0('sig_sets_ImmDict_cytokine_family_order_by_max_log10Padj.txt'))
