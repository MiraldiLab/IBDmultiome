...
file_gsea_ImmDict<-file.path(results_dir,paste('Downsample/1Factor_CellType_Downsample_RNAEnrich/CD_TI/final_ImmDict_max200/gsea_IBD_multiome_final_ImmDict_max200_I_vs_NI_Disease_Tissue_All.rds'))
file_sig_sets_ImmDict<-file.path(results_dir,paste('Downsample/1Factor_CellType_Downsample_RNAEnrich/CD_TI/final_ImmDict_max200/sig_setsIBD_multiome_final_ImmDict_max200_All.txt'))
file_gsea_PROGENy_CD64<-file.path(results_dir,paste('Downsample/1Factor_CellType_Downsample_RNAEnrich/CD_TI/final_PROGENy_CD64_max200/gsea_IBD_multiome_final_PROGENy_CD64_max200_I_vs_NI_Disease_Tissue_All.rds'))
file_sig_sets_PROGENy_CD64<-file.path(results_dir,paste('Downsample/1Factor_CellType_Downsample_RNAEnrich/CD_TI/final_PROGENy_CD64_max200/sig_setsIBD_multiome_final_PROGENy_CD64_max200_All.txt'))
file_df_frac<-file.path(data_dir,paste('df_TPM_frac_exp_by_CellType_Disease_Tissue_Status.rds'))
...
# output directories
dir_out <- file.path(project_dir, "results","GSEA_RNA_Status", "Downsample_RNAEnrich","PROGENy_CD64_ImmDict", "CD_TI")
...
df_frac<-df_frac[df_frac$DiseaseTissue=='CD_TI',]
...
colnames(df_ImmDict)<-c('Cytokine','NGene','-log10Padj','coeff','odds.ratio','Context')
...
df_frac_ligand_disease_tissue_status<-df_frac %>%
  group_by(Gene, DiseaseTissue, Status) %>%
  summarise(max_min5pct = max(min5pct, na.rm = TRUE), .groups = 'drop')
...
for (ix in 1:nrow(df_ImmDict)){
  ...
  curr_min_5pct_gene_celltype_context<-df_frac[df_frac$Context%in%curr_context&df_frac$Status%in%curr_status&df_frac$min5pct==1,'Gene']
  ...
  curr_min_5pct_gene_context<-df_frac_ligand_disease_tissue_status[df_frac_ligand_disease_tissue_status$Status%in%curr_status&df_frac_ligand_disease_tissue_status$max_min5pct==1,'Gene']
  df_ImmDict[ix,'Ligand_5pct_min']<-ifelse(all(curr_ligand %in% curr_min_5pct_gene_context), 1, 0)
}
...
df_pathway_ImmDict<-df_ImmDict %>%
  group_by(Cytokine, Pathway, Context,`-log10Padj`, NGene,coeff, odds.ratio,Status) %>%
...
colnames(df_PROGENy_CD64)<-c('Cytokine','NGene','-log10Padj','coeff','odds.ratio','Context')
...
for (ix in 1:nrow(df_PROGENy_CD64)){
  ...
  if(length(curr_ligand)>0){
    curr_min_5pct_gene_context<-df_frac_ligand_disease_tissue_status[df_frac_ligand_disease_tissue_status$Status%in%curr_status&df_frac_ligand_disease_tissue_status$max_min5pct==1,'Gene']
    df_PROGENy_CD64[ix,'Ligand_5pct_min']<-ifelse(all(curr_ligand %in% curr_min_5pct_gene_context), 1, 0)
  }else{
